...
Решение
p = {p}; q = {q}; n = {7}
P_n(k) = C_n^k * p^k * q^(n-k), k = 1..n - формула Бернули
...
SUM_(i=0)^({k-1}) (P_{n}(i)) = P_{n}(0) + ... + P_{n}({k-1}) = 
= {out_1_k_1} = {sum_1_k_1}

Вероятность того, что шар выпадет не более {k-1} раз равна {sum_1_k_1}
Вероятность того, что шар выпадет не менее {k} раз равна: 1 - SUM_(i=0)^({k-1}) (P_{n}(i)) = {1-sum_1_k_1} - ответ

!СЛУЧАЙ, КОГДА k > n/2! ({k} > {n//2} - {k > n/2})
Посчитаем вероятности попадания ровно k, ... , n ({k}, ..., {n}) раз.
SUM_(i={k})^({n}) (P_{n}(i)) = P_{n}({k}) + ... + P_{n}({n}) = {out_k_n} = {sum_k_n}

Вероятность того, что шар выпадет не менее {k} раз равна: SUM_(i={k})^({n}) (P_{n}(i)) = {sum_k_n} - ответ

в)
SUM_(i={k1})^({k2}) = P_{n}({k1}) + ... + P_{n}({k2}) = {out_k1_k2} = {sum_k1_k2}
